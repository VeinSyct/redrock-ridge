...
let zoomOnScreen = (c, d) => {
    if (_pm.mobile) {
        if ((c == true && magnify.enabled == "once") || (c == "once" && c != magnify.enabled && magnify.enabled == true)) {
            magnify.enabled = c;
        } else return;
        if (magnify.enabled) {
            if (magnify.mobile) {
                ...
                delete magnify.grow;
            } else snapRender((magnify.grow = magnify.mobile = true));
        }
    ...
    register.crowd = {
        ...
        interval: setInterval(async () => {
            ...
            if (register.crowd.behaviour[d.uid]) {
                ...
                } else _pm.model.loadAvatarAnimation({ name: d.do.actions[Math.floor(Math.random() * d.do.actions.length)], mesh: d.mesh, loop: false, fadeIn: .42 });
            }
...
showSprites = (op) => {
        ...
        if (!op.retour && !op.toTour) {
            if (_pm.p.hiddenSprites)
                for (let i = 0; i < _pm.p.hiddenSprites.length; i++) {
                    ...
                    if (op.mesh) op.mesh.visible = true;
                }
            _pm.p.hiddenSprites = [];
            if (op.hide)
    ...
    register.animation.update = register.loaded = true;
},
        ...
        if (d[j[i]])
            for (let k = 0, l; k < d[j[i]].length; k++) {
                ...
                if (l) {
                    ...
                    _pm.p.avatar.enabled = j[i].match(/(hide)/) ? 1 : false;
            ...
            .load(_pm.p.models[d.index].shadow.name, (texture) => {
                if (_pm.p.models) {
                    register.mesh = new THREE.Mesh(
                        ...
                        new THREE.MeshBasicMaterial({ map: texture, blending: THREE.MultiplyBlending, toneMapped: false, transparent: true, premultipliedAlpha: true })
                    );
...
setAudio = (d) => {
    if (!d.mesh.userData.audio)
        scene.add((d.mesh.userData.audio = new THREE.PositionalAudio(camera.userData.listener)));
    ...
    if (d.pos.y > -1e3) {
        ...
        if (!d.mesh.userData.audio.loaded) {
            d.mesh.userData.audio.loaded = true;
            ...
            if (!d.halt) {
                ...
                for (let k in d.positions) {
                    if (_pm.p.external.filter.includes(k)) {
                        ...
                        _pm.tracking.object[k].visible = true;
                ...
                if (d.wa > (_pm.mobile ? 310 : 264) || d.wa < (_pm.mobile ? 24 : 42)) {
                    ...
                    if (_pm.tracking.misAligned === undefined) {
                        _pm.tracking.misAligned = true;
                        window.parent.postMessage({ code: codeString, action: "mis-aligned", twisted: true }, "*");
                    }
                }
            } else {
                d.m.visible = false;
...
window.externalClicked = (d) => {
    if (d.type == "pane") {
        ...
        if (register.animated && register.animated.sprites) {
            register.animated.sprites.preview = true;
        } else register.animated.sprites = { preview: true };
    }
    if (d.type == "dot") {
        if (_pm.p.external) {
            ...
            if (_pm.p.external.animations && _pm.p.external.animations[d.name]) {
                for (let i = 0; i < _pm.p.external.animations[d.name].length; i++) {
                    ...
                    staticPanes({ n: d.name, i: i, hide: true });
                }
            }
...
window.skipAutoPlay = (c, nb) => {
    ...
    if (c && !nb) {
        showCenterIcon({ display: "none", lock: true });
        delete register.clicked;
        delete register.views;
        register.views = getViewsSettings(true);
    } else {
        glowSelectedParts();
...
window.loadProgram = async (d) => {
    ...
    if (_pm.p.camera) {
        ...
        if (_pm.mobile) register.focus = true;
    ...
    for (let i = 0, j = ["markers", "sprites"]; i < j.length; i++)
        if (_pm.p[`${j[i]}Data`])
            for (let key in _pm.p[`${j[i]}Data`].materials) {
                if (_pm.p[`${j[i]}Data`].materials[key].map) _pm.p[`${j[i]}Data`].materials[key].map.needsUpdate = true;
                if (_pm.p[`${j[i]}Data`].materials[key].alphaMap) _pm.p[`${j[i]}Data`].materials[key].alphaMap.needsUpdate = true;
            }
};
window.onLoadSkybox = (op) => {
    ...
    if (op !== undefined) {
        ...
        lerpToTarget({ start: _pm.p.camera.touring.zoom.arriving, final: _pm.p.camera.zoom, rate: _pm.p.camera.touring.rate.arriving, zoom: false });
    }
};
window.cubeBox = (i) => {
...
window.exploreModel = (op) => {
    register.switching = true;
    showCenterIcon({ lock: true });
    ...
    loadProgram({ preloaded: (_pm.p.models[op.model].preloaded = true) });
    _pm.p.models[(_pm.p.mIndex.selected = op.model)].preloaded = false;
    if (_pm.p.hiddenSprites) {
...
window.exploreTours = (op) => {
    if (!register.lastTour.toTour) lerpToTarget({ start: _pm.p.camera.zoom, final: _pm.p.camera.touring.zoom.exiting, rate: _pm.p.camera.touring.rate.exiting, zoom: true });
    if (register.lastTour) {
...
window.lerpToTarget = (d) => {
    ...
    register.lerpInterval = setInterval(() => {
        ...
        if ((d.start < d.final * .99 && !d.zoom) || (d.start > d.final * .99 && d.zoom)) {
            if (postProcessing && d.start < 1 && !d.updated) onWindowResize({ updated: (d.updated = true) });
            d.start = (1 - d.rate) * d.start + d.rate * d.final * (d.zoom ? -1 : 1);
            d.lerping = register.animation.update = true;
                    ...
                    if (!loader.gSkybox.busy) {
                        ...
                        (c.onMax.texture.name.includes("jpg") ? new THREE.TextureLoader() : new RGBELoader())
                            ...
                            .load(c.onMax.texture.name, (texture) => {
                                ...
                                loader.gSkybox.texture = { map1: loader.gSkybox.dome.material.map, map2: texture, isMap2: true };
    ...
    if (event.type.match(/(message)/))
        try {
            if (event.data.action) {
                ...
                if (event.data.action == "play") skipAutoPlay(true);
                if (event.data.action == "glow") glowSelectedParts(true);
                ...
                if (event.data.action.match(/(user-inputs)/)) {
                    if (register.physics.enabled) {
                        ...
                        if (register.user.vehicle[_pm.user.uid] && (event.data.inputs.keyEnter || event.data.inputs.keyESC)) _pm.physics.switchDriving({ uid: _pm.user.uid, drive: event.data.inputs.keyEnter ? true : false });
                ...
                if (event.data.action.match(/(action-)/) && register.user.uid[_pm.user.uid]) {
                    ...
                    if (event.data.action.match(/(wave-hand)/)) {
                        ...
                        if (event.mesh.userData.isDriving || event.mesh.userData.isPassenger) {
                            ...
                            loadAudio({ mesh: register.user.vehicle[_pm.user.uid].mesh, audio: event.aH[Math.floor(Math.random() * event.aH.length)], loop: false });
                        } else {
                            event.acts = register.avatar.activities.waving;
                            _pm.model.loadAvatarAnimation({ name: event.acts[Math.floor(Math.random() * event.acts.length)], mesh: event.mesh, loop: false, fadeIn: .42 });
...
window.areAnimatedVisible = (d) => {
    ...
    if (!_pm.model.iBusy)
        isInteriorVisible({ k: 0, b: (_pm.model.iBusy = true) });
};
window.orbitEvent = (event) => {
    ...
    if (event.type.match(/(end)/)) {
        if (register.events && !register.events.orbiting) register.events.orbiting = true;
        areAnimatedVisible({});
...
window.rendererEvent = (event) => {
    ...
    if (event.type.match(/(mousedown|wheel)/)) register.views.completed = true;
                                        ...
                                        if (intersects[0].object.name.match(/(markers-|sprites-)/)) {
                                            ...
                                            if (op) {
                                                ...
                                                if (op.name.match(/(tour-360|to-tour-360|explore-model)/)) {
                                                    lerpToTarget({ start: _pm.p.camera.zoom, final: _pm.p.camera.touring.zoom.departing, rate: _pm.p.camera.touring.rate.departing, zoom: false });
...
window.dynamicMeshClicked = (event) => {
    ...
    if (event.interactive && event.interactive.values && event.interactive.values[event.parent.name])
        if (event.interactive.operation == "replace model") {
            ...
            if (event.sign) event.sign.visible = false;
            showCenterIcon({ image: "loading.webp", height: "64px", opacity: .96, display: "flex", delay: 120 });
            ...
            event.sign = scene.getObjectByName(`sign-${_pm.p.models[_pm.p.mIndex.selected].name.toLowerCase().replace(_pm.p.dynamic.replace, "")}`);
            if (event.sign) event.sign.visible = true;
            event.mesh = scene.getObjectByName("mesh shadow");
            if (event.mesh) _pm.model.disposeHierarchy(event.mesh);
            window.parent.postMessage({ code: codeString, action: "model-changed", name: _pm.p.models[_pm.p.mIndex.selected].name }, "*");
            register.views.index = register.views.positions.sequence[loader.pIndex].length - 1;
            ...
            setTimeout(() => {
                if (register.events[event.type]) {
                    if (_pm.p.dynamic.animation.hovering.onetime === undefined || _pm.p.dynamic.animation.hovering.onetime) {
                        if (_pm.p.dynamic.animation.hovering.onetime) _pm.p.dynamic.animation.hovering.onetime = false;
                        ...
                        for (let k = 0, l = _pm.p.dynamic.screens, m, n, o; k < l.length; k++)
                            if (register.sidePairsMaterials[register.events[event.type].hovering + l[k]]) {
                                ...
                                if (n && n.images) {
                                    ...
                                    if (!o.status) {
                                        o.status = true;
                                        register.animated.visible = true;
...
window.unloadModels = (d) => {
    requestIdleCallback(() => {
        ...
        for (let i in d.models) {
            while (_pm.p.models[d.models[i]] && (d.mesh = _pm.model.getMesh({ p: "name", k: _pm.p.models[d.models[i]].name }))) {
                if (d.mesh.mIdx == d.models[i]) {
                    ...
                    if (d.mesh.userData.audio)
                        scene.remove(d.mesh.userData.audio);
                    if (_pm.p.social.users[d.models[i]]) {
                        delete _pm.p.social.users[d.models[i]];
                        delete _pm.p.models[d.models[i]];
                    }
                    if (d.all)
...
window.loadModelCompleted = (url, loaded, total) => {
    if (loaded / total == 1) {
        if (register.switching) {
            ...
            lerpToTarget({ start: _pm.p.camera.touring.zoom.arriving, final: _pm.p.camera.zoom, rate: _pm.p.camera.touring.rate.arriving, zoom: false });
            showCenterIcon({ lock: false });
        } else window.parent.postMessage({ code: codeString, action: "hide-waiting" }, "*");
        register.loaded = register.animation.update = true;
        if (_pm.p.camera.fixed || register.physics) {
            showCenterIcon({ display: "none", lock: false, delay: register.physics ? 4096 : 1200 });
        ...
        if (register.physics) {
            if (register.physics.ready && !register.physics.enabled)
                setTimeout(() => {
                    _pm.worker.aiPhysics.operation.postMessage({ action: "physics-enabled", enabled: (register.physics.enabled = true), opacity: (_pm.screen.style.opacity = 1) });
                    window.parent.postMessage({ code: codeString, action: "enable-controls" }, "*");
                    _pm.model.isInBoundaries({ b: _pm.model.boundaries, ready: true, zn: "enter-zone" });
        ...
        for (let i = 0, j = ["enter", "leave"]; i < j.length; i++)
            if (register.unload[j[i]] && register.unload[j[i]].length > 0)
                unloadModels({ models: register.unload[j[i]], reset: register.unload[j[i]] = [], all: true });
        register.loading = [];
...
window.loadModel = async (d) => {
    if (_pm.p.models[d.index].userData) {
        if (d.px = _pm.p.models[d.index].userData.proximity) {
            ...
            if (d.p.distanceTo(camera.position) > d.px.distance && (!d.px.occluded && !isPositionInView({ p: d.p }))) {
                ...
                for (let i in d.px.impostor)
                    _pm.p.models[d.px.impostor[i]].preloaded = true;
                return;
            }
        ...
        if (!_pm.p.models[d.index].userData.loaded)
            _pm.p.models[d.index].userData.loaded = true;
        else return;
    }
    ...
    if (isOnline() && !_pm.useWebGPU && !loader.useNeedle && (loader.useNeedle = true)) {
...
window.processGLTF = (d, gltf) => {
    gltf.scene.traverse(async (child) => {
        if (child.name.match(/(intersect|phy-body-main|phy-body-part|phy-body-vehicle)/)) {
            child.visible = false;
            if (child.material) { child.material.color = 0x000000; child.material.wireframe = true }
            ...
            if (child.name) {
                for (let i = 0, j = ["hide", "show"]; i < j.length; i++)
                    if (_pm.p.models[d.index][j[i]] && _pm.p.models[d.index][j[i]].includes(child.name)) {
                        d.show = child.name;
                        child.visible = j[i].match(/(hide)/) ? false : true;
                    }
            ...
            if (child.isMesh) {
                ...
                if (!_pm.useWebGPU && (!_pm.p.dynamic || (_pm.p.dynamic && _pm.p.dynamic.videoTextures && !_pm.p.dynamic.videoTextures.includes(child.material.name)))) {
                    ...
                    child.userData.culling = _pm.p.models[d.index].culling;
                }
            }
            if (child.material) {
                ...
                if (d.m) {
                    ...
                    for (let i = 0; i < textMaps.length; i++)
                        if (child.material[textMaps[i]]) {
                            ...
                            child.material[textMaps[i]].generateMipmaps = true;
                ...
                try {
                        for (let k = 0, l = ["videos", "interiors"]; k < l.length; k++)
                            if (_pm.p.models[d.index][l[k]]) {
                                if (l[k].match(/()/)) d.isAnimated = true;
    ...
    if (_pm.p.physics && _pm.p.models[d.index].physics) {
        ...
        if (_pm.p.models[d.index].isAvatar)
            _pm.worker.aiPhysics.operation.postMessage({
                action: `${_pm.p.models[d.index].isAvatar?.isPlayer ? "main" : "other"}-player`,
                id: gltf.scene.phyID
            });
        ...
        gltf.scene.traverse((child) => {
            ...
            if (child.name.match(/(phy-boundary)/)) {
                ...
                d.cg.attributes.position.needsUpdate = true;
                d.be = _pm.math.getBoundaryEdges({ a: child.geometry.attributes.position, i: child.geometry.index, e: [], s: [] });
                if (false) {
                    _pm.math.traceBoundery({ boundaryEdges: d.be, color: 0xff0000, y: 10, isFrom: true });
                ...
                _pm.model.boundaries[`${child.name.replace(/phy-boundary-/g, "")}-${d.rID}`] = {
                    entering: gltf.scene.userData.entering ? structuredClone(gltf.scene.userData.entering) : false,
        ...
        if (d.rs) {
            ...
            if (!_pm.model.isInBoundaries({ b: _pm.model.boundaries, zn: "respawn-near" })) {
                ...
                if (d.a && (!register.physics.enabled && d.a.userData.isFalling))
                    _pm.physics.revertUser({ phyID: d.a.phyID.replace(/-main/g, ""), p: _pm.p.avatar.revert.position, r: _pm.p.avatar.revert.rotation, rs: _pm.p.avatar.revert.respawn, re: true });
            }
        }
    ...
    if (_pm.p.camera.isAugmented && smoothedRoot) {
        if (_pm.p.models[d.index].arDebug) smoothedRoot.add(new THREE.Mesh(new THREE.BoxGeometry(1, 1, 1), new THREE.MeshNormalMaterial({ transparent: true, opacity: .5, side: THREE.DoubleSide })));
    ...
    if (d.isAvatar) {
        ...
        setAvatarBehaviours({ name: "phyID", value: gltf.scene.phyID, isIdle: true });
        d.isAnimated = true;
    }
    ...
    if (_pm.p.models[d.index].userData) {
        ...
        if (_pm.p.models[d.index].userData.archive) {
            ...
            if (!_pm.model.aBusy && register.busy.archived.now + 1536 < performance.now())
                _pm.model.reloadArchived({ now: register.busy.archived.now = performance.now(), a: _pm.model.archived, x: true, i: 0 });
        }
        if (_pm.p.models[d.index].userData.animation) {
            if (_pm.p.models[d.index].userData.animation.play) {
                _pm.model.playMeshAnimation({ parent: _pm.p.models[d.index].name, any: true, config: {
    ...
    if (_pm.p.models[d.index].userData && _pm.p.models[d.index].userData.unload)
        ...
        if (_pm.p.avatar && _pm.p.avatar.revert.reposition) {
            ...
            if (d.uD.isDriving || d.uD.isPassenger)
                _pm.physics.switchDriving({ uid: _pm.user.uid, drive: false });
            setTimeout(() => {
...
window.createSpriteMaterial = (texture, cutout, i, typ) => {
    ...
    _pm.p[`${typ}Data`].materials[_pm.p[typ][i].image.name] =
        typ == "sprites" ? new THREE.SpriteMaterial({ map: texture, alphaMap: cutout }) : new THREE.MeshBasicMaterial({ map: texture, alphaMap: cutout, transparent: _pm.p[typ][i].image.opaque ? false : true });
    return i;
};
        ...
        if (Object.keys(register.animated.colors).length > 0) {
            for (let i in register.animated.colors) {
                ...
                if (!register.animated.colors[i].material[register.animated.colors[i].mapName].equals(register.animated.colors[i].color)) {
                    register.animated.colors[i].material.needsUpdate = true;
            ...
            if (!_pm.p.avatar || (_pm.p.avatar && register.avatar.camera)) {
                try {
                    ...
                    if (register.animation.error) {
                        ...
                        register.error.modal.style.display = "none";
                    ...
                    if (!register.animation.error) {
                        ...
                        register.error.modal.innerHTML = `
                            ...
                            <div class="error-modal-message">
                                ...
                                <p>Please wait a moment, or refresh the page.</p>
                            </div>`;
                        register.error.modal.style.display = "block";
                    }
                }
            }
            if (!_pm.useWebGPU) {
        ...
        if (!d.mesh.userData.isDriving && !d.mesh.userData.isPassenger) {
            if (d.isIdle || !d.acts) {
                if (register.avatar.meshes[register.user.uid[_pm.user.uid]] && d.mesh.name == register.avatar.meshes[register.user.uid[_pm.user.uid]].name) {
                    if (d.in = register.user.inputs[_pm.user.uid])
            ...
            if (register.invideos)
                for (let key in register.invideos.e)
                    if (register.invideos.data[key] && !register.invideos.data[key].material.userData.noplay)
                        register.invideos.data[key].material.userData.paused = true;
            if (register.invideos)
                for (let key in register.invideos.e)
                ...
                for (d.i = 0; d.i < d.animated.length; d.i++)
                    if (d.animated[d.i]) {
                        ...
                        if (d.m) {
                            d.frustum = new THREE.Frustum().setFromProjectionMatrix(new THREE.Matrix4().multiplyMatrices(camera.projectionMatrix, camera.matrixWorldInverse));
                            d.m.traverse((child) => {
                                if (child.isMesh && ((d.frustum.containsPoint(child.position) && false) || d.frustum.intersectsObject(child)))
                                    for (d.j = 0; d.j < d.find.length; d.j++) {
                                        ...
                                        } else if (child.name == d.find[d.j].sName) register.animated.visible = true;
                                    }
                            });
                        }
                    }
...
window.hideCursorPointer = (d) => {
    if (!register.physics || (register.physics && register.physics.enabled)) {
        ...
        register.views.completed = register.clicked = true;
    }
};
